# El-Abd branding
Rebranding project for El Abd .   
This repository contains research, design concepts, brand strategy, visual identity development and and packaging design.

<img width="512" height="512" alt="unnamed" src="https://github.com/user-attachments/assets/96baba3c-631e-49a8-88f7-9204b44d9ed0" /> 
 
Team Member :

1-Maha Abo Elkhier Abd Elstar Mohammad

2-Mariam Gehad Fathy Gomaa

3-Basmala Mostafa Ahmed Mahrous

4-Iten Hossam Ibrahim Saad Eldin

5-Marwa Saber Saleh Hassanein

📌 Project Overview

El Abd Patisserie is one of the most iconic dessert brands in Egypt, known for its heritage and popularity. This project aims to create a comprehensive rebranding that preserves the brand’s legacy while modernizing its visual identity to appeal to both loyal customers and new generations. The rebranding will cover all touchpoints — from logo and packaging to digital presence — ensuring a cohesive, memorable, and engaging brand experience.

🎯 Project Objectives

Modernize the brand image while respecting its heritage and authenticity.
Develop a distinctive logo that communicates quality, trust, and tradition.
Redesign packaging to be visually appealing, functional, and consistent with the new brand identity.
Create a brand character that adds a human and relatable touch to the brand story.
Establish a full visual identity system (colors, typography, icons, guidelines).
Design engaging marketing materials such as posters, menus, and social media content to strengthen brand communication.
Ensure consistency across all brand applications (print, digital, physical).

📦 Project Scope

The project includes the following deliverables:
Logo Design: Primary logo, variations, and logo usage guidelines.
Packaging Design: Boxes, bags, labels, and seasonal editions.
Character Design: A unique mascot/character to represent the brand.
Visual Identity System: Color palette, typography, grid systems, brand book.
Menu Design: Redesigned menus for in-store and digital use.
Posters & Print Materials: Advertising, promotions, and campaigns.
Social Media Posts: Templates, campaign visuals, and digital assets.

🗂️ Project Plan

Phase 1 – Research & Analysis
Brand audit (current identity, competitors, target audience).
Collect inspiration & mood boards.

Phase 2 – Concept Development
Logo sketches and explorations.
Packaging moodboards & initial concepts.
Character design drafts.

Phase 3 – Design & Refinement
Finalize logo and variations.
Packaging prototypes.
Character final design.
Create the full visual identity system.

Phase 4 – Applications
Menu design.
Posters & marketing collaterals.
Social media post templates and visuals.

Phase 5 – Documentation & Presentation
Develop the brand guidelines (brand book).
Prepare final presentation showcasing the rebranding journey.

Division of roles:
1. Logo Design : Maha ِAbo Elkhier
2. Character Design: Maha Abo Elkhier
3. Social Media Posts: Maha Abo Elkhier -> Design 1&2
                       Basmala Mostafa -> Design 3&5
                       Salma Ahmad -> Design 4
4. Billboard Posters : Maha Abo Elkhier -> Design 1&2
                       Iten Hossam -> Design 3
                       Salma Ahmad -> Design 4
5. Menu Design: Maha Abo Elkhier -> Design 1&2
                Mariam Gehad -> Design 3&4
6. Packaging Design: Maha Abo Elkhier
7. Flyers : Basmala Mostafa -> Design 1
            Marwa Saber -> Design 2

https://drive.google.com/drive/folders/1Gmw8IY0u3qoDoGN97U5B9hjLBRvZRxOK?usp=drive_link
